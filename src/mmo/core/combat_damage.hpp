...
namespace mmo
{
    namespace core
    {
        namespace combat
        {
            // Combat damage contract: profile inputs and resolved output.
            ...
            // Resolves criticals, reductions, and shields into final damage output.
            [[nodiscard]] inline auto resolve_damage(
                const DamageProfile& profile,
                const DefenseProfile& defense) -> DamageResult
            {
                auto result = DamageResult{};
                result.source = profile.packet.source;
                ...
                result.shield_absorbed = std::min(
                    std::max<std::int32_t>(0, defense.shield_current),
                    mitigated_damage);

                result.health_damage = mitigated_damage - result.shield_absorbed;

                result.shield_broken =
                    result.shield_absorbed > 0 &&
                    defense.shield_current > 0 &&
                    result.shield_absorbed >= defense.shield_current;

                return result;
            }

            [[nodiscard]] inline auto to_damage_trace(const DamageResult& result) -> damage::Trace
            {
                damage::Trace trace{};
                trace.source = result.source;
                trace.kind = result.kind;
                trace.incoming_damage = result.incoming_damage;
                trace.mitigated_damage = result.mitigated_damage;
                trace.shield_absorbed = result.shield_absorbed;
                trace.health_damage = result.health_damage;
                trace.critical = result.critical;
                trace.shield_broken = result.shield_broken;
                return trace;
            }

            inline auto record_damage(
                damage::Ledger& ledger,
                const DamageResult& result) -> void
            {
                ledger.record(to_damage_trace(result));
            }
        }
    }
}